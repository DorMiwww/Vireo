...
| IntelliJ Plugin SDK approval + scaffold | User approved adding the IntelliJ Platform Plugin SDK. Built `vireo-idea-plugin/` as a standalone Gradle project (own wrapper, Gradle 9.0.0 / Kotlin 2.1.0 / IntelliJ Platform Gradle Plugin 2.19.0) — file type + icon, `DacLexerAdapter` wrapping `vireo-lexer`, `DacAnnotator` calling `vireo-parser`/`vireo-analysis` in-process, `DacCompletionContributor` with a minimal flat-PSI `DacParserDefinition`. Consumes `vireo-core`/`vireo-lexer`/`vireo-parser`/`vireo-analysis` as plain jars (`sync-libs.sh`), not project deps | 2026-09-20 |
| IntelliJ plugin build verified | `./gradlew compileKotlin`, `buildPlugin`, `verifyPluginStructure` all pass against real IntelliJ Platform SDK (IC 2024.3) — produces installable `vireo-idea-plugin-0.1.0-SNAPSHOT.zip`. Needed 3 settings/build-script fixes found only by actually running the build: missing `import ...intellijPlatform` in `settings.gradle.kts`, duplicate plugin version between settings- and project-level `plugins{}`, and a `repositories{}` block in `build.gradle.kts` conflicting with `FAIL_ON_PROJECT_REPOS`. Also added the `foojay-resolver-convention` plugin so the JDK 17 toolchain auto-downloads (this machine only has JDK 24) | 2026-09-20 |
